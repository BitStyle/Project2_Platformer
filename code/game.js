...
var actorChars = {
  ...
  "y": Enemy,
  "=": Lava, "|": Lava, "v": Lava,
  "s": TeleporterStart, "f": TeleporterFinish,
  "w": Flag
};
var teleporterFinishLocation = new Vector(0,0);
function Level(plan) {
...
// Add another new actor type as a class
function Enemy(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.6, 0.6);
  // Make it go back and forth in a sine wave.
...
// Add another new actor type as a class
function TeleporterStart(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.6, 0.6);
}
TeleporterStart.prototype.type = "teleporterStart";

// Add another new actor type as a class
function TeleporterFinish(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  teleporterFinishLocation = this.basePos;
  this.size = new Vector(0.6, 0.6);
}
TeleporterFinish.prototype.type = "teleporterFinish";
...
var enemyMaxStep = 0.1;

var enemyWobbleSpeed = 2, enemyWobbleDist = 1;

Enemy.prototype.act = function(step) {
  this.wobble += step * enemyWobbleSpeed;
  var enemyWobblePos = Math.cos(this.wobble) * enemyWobbleDist;
  this.pos = this.basePos.plus(new Vector(enemyWobblePos, 0));
};
...
TeleporterStart.prototype.act = function(step) {
this.pos = this.basePos;
}

TeleporterFinish.prototype.act = function(step) {
this.pos = this.basePos;
}
...
Player.prototype.act = function(step, level, keys) {
  ...
  if(level.status == "teleportStart"){
    this.pos = teleporterFinishLocation;
    level.status = "teleported";
  }
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // Player loses
  if ((type == "lava" || type == "enemy") && (this.status == null || this.status == "teleported")) {
    this.status = "lost";
    this.finishDelay = 1;
  }
  else if (type == "teleporterStart") {
    this.status = "teleportStart";
  }
  else if (type == "coin") {
